...
function HomePage() {
  ...
  return (
    <section className="space-y-8">
      <article className="relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white/90 p-5 shadow-lg shadow-slate-200/50 backdrop-blur transition-colors duration-300 sm:p-6 dark:border-slate-800/90 dark:bg-slate-900/85 dark:shadow-slate-950/55">
        <div className="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-emerald-100/60 blur-3xl dark:bg-emerald-900/30" />
        <div className="pointer-events-none absolute -left-10 bottom-0 h-40 w-40 rounded-full bg-cyan-100/50 blur-3xl dark:bg-cyan-900/25" />
        ...
        <div className="relative">
          <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-xl font-extrabold tracking-tight text-slate-900 sm:text-2xl dark:text-slate-100">
            ...
            <button
              ...
              className={[
                ...
                hasActiveFilters
                  ? 'border-slate-300 bg-white text-slate-700 hover:-translate-y-0.5 hover:border-slate-900 hover:text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:border-teal-300 dark:hover:text-teal-200'
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label className="block rounded-2xl border border-slate-200 bg-white/90 p-3 transition-all duration-200 focus-within:-translate-y-0.5 focus-within:border-slate-400 focus-within:shadow-md focus-within:shadow-slate-200/70 dark:border-slate-700 dark:bg-slate-800/80 dark:focus-within:border-teal-400 dark:focus-within:shadow-teal-950/60">
              ...
              <input
                ...
                className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-slate-900 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-teal-300"
              />
              <span
                className={[
                  'mt-1 block text-xs text-slate-500 transition-opacity duration-200 dark:text-slate-400',
            ...
            <label className="block rounded-2xl border border-slate-200 bg-white/90 p-3 transition-all duration-200 focus-within:-translate-y-0.5 focus-within:border-slate-400 focus-within:shadow-md focus-within:shadow-slate-200/70 dark:border-slate-700 dark:bg-slate-800/80 dark:focus-within:border-teal-400 dark:focus-within:shadow-teal-950/60">
              ...
              <select
                ...
                className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-slate-900 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-teal-300"
              >
      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        <aside className="relative overflow-hidden rounded-3xl border border-slate-200/90 bg-white/95 p-4 shadow-lg shadow-slate-200/45 transition-colors duration-300 sm:p-5 dark:border-slate-800/90 dark:bg-slate-900/90 dark:shadow-slate-950/55">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-slate-50 to-transparent dark:from-slate-800/60" />
          ...
          {isSearchPending ? (
            <div className="mb-3 h-1 w-full overflow-hidden rounded bg-slate-100 dark:bg-slate-800">
              <div className="h-full w-1/3 animate-soft-pulse rounded bg-slate-400 dark:bg-teal-300" />
            </div>
          ) : null}
          ...
          {listLoading ? (
            <div className="space-y-2" role="status" aria-label="Loading country list">
              {Array.from({ length: listSkeletonRows }).map((_, index) => (
                <div
                  key={`skeleton-${index}`}
                  className="h-11 w-full animate-pulse rounded-xl bg-gradient-to-r from-slate-100 via-slate-200/80 to-slate-100 dark:from-slate-800 dark:via-slate-700 dark:to-slate-800"
                />
              ))}
            </div>
          ) : null}
          ...
          {!listLoading && filteredCountries.length === 0 ? (
            <p className="text-sm text-slate-500 dark:text-slate-400">No countries match your filters.</p>
          ) : null}
